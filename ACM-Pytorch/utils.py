...
def random_disassortative_splits(labels, num_classes):
    ...
    # 稀疏分割
    # percls_trn = int(round(0.025 * (labels.size()[0] / num_classes)))
    # val_lb = int(round(0.025 * labels.size()[0]))
...
def train_model(
    ...
    model.train()
    optimizer.zero_grad()
    output = model(features, adj_low, adj_high, adj2_low, adj2_high,fadj_low,fadj_high, adj_low_unnormalized)
...
def train_prep(logger, args):
    ...
    fadj = get_top_k_matrix(feature_matrix,k=args.k)
   
    adj_low_unnormalized = adj_low_unnormalized.to_dense()+fadj.to_dense()  # my
    ...
    if args.structure_info:
        ...
        adj_low = normalize_tensor(torch.eye(nnodes) + adj_low_unnormalized)
        ...
        adj2 = adj_low_unnormalized
        adj2_low =  normalize_tensor(adj2 + torch.eye(nnodes))
        adj2_high = (torch.eye(nnodes) - adj2_low).to(device).to_sparse()
        adj2_low = adj2_low.to(device)

        fadj_low = normalize_tensor(torch.eye(nnodes) + fadj.to_dense())
        fadj_high = (torch.eye(nnodes) - fadj_low).to(device).to_sparse()
        fadj_low = fadj_low.to(device)
...
def load_graph(args):
    featuregraph_path ="../data/knn/" + args.dataset_name  + "/c" + str(args.k) + '.txt'
    feature_edges = np.genfromtxt(featuregraph_path, dtype=np.int32)
    fedges = np.array(list(feature_edges), dtype=np.int32).reshape(feature_edges.shape)
    fadj = sp.coo_matrix((np.ones(fedges.shape[0]), (fedges[:, 0], fedges[:, 1])), shape=(args.n, args.n), dtype=np.float32)
...
def get_disfeature_matrix(x):
    fMatrix = x.cpu()
    feature_matrix= pairwise_distances(fMatrix)

    return feature_matrix

def get_top_k_matrix(feature_matrix: np.ndarray, k: int = 10) -> np.ndarray:
    num_nodes = feature_matrix.shape[0]
    row_idx = np.arange(num_nodes) 
    feature_matrix[feature_matrix.argsort(axis=0)[:num_nodes - k], row_idx] = 0.  #A.argsort(axis=0) 沿着行向下(每列)的元素从小到大进行排序，返回行坐标
    ...
    adj_coo = torch.sparse_coo_tensor(idx, data, feature_matrix.shape) 
    return adj_coo
